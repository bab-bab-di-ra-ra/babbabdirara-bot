...
def get_bab_comment(menu_list, api_key):
    ...
    response = client.chat.completions.create(
        ...
        messages=[{
            "role": "user",
            "content": f"오늘 점심이 {menu_text}래. '밥밥디라라 등장!' 느낌으로 2000년대 인터넷 개그 감성 한마디만 해줘. 시스템 경고나 위험도 분석 느낌을 살리고, 이모지 1개 포함해서 1줄로 써줘. 학교 식단이므로 술(소주, 맥주, 와인 등)이나 음주 관련 표현은 절대 사용하지 마. 물결표(~)는 Teams에서 취소선으로 보일 수 있으니 절대 사용하지 마. 모든 문장은 한국어로만 작성하고, 영어, 러시아어, 일본어, 중국어 등 다른 언어를 섞지 마."
        }]
    )
